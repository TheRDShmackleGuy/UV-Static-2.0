...
class UVServiceWorker extends EventEmitter {   
    ...
    async fetch({ request }) {
        ...
        try {
            if (request.method === 'GET' && ['image', 'style', 'script', 'font'].includes(request.destination)) {
                const key = `${request.method}:${request.destination}:${request.url}`;
                const cached = uvInflight.get(key);
                if (cached && (Date.now() - cached.ts) < UV_INFLIGHT_TTL) {
                    const resp = await cached.promise;
                    return resp.clone();
                }
                let resolveResp;
                const p = new Promise((resolve) => { resolveResp = resolve; });
                uvInflight.set(key, { ts: Date.now(), promise: p });
                const cleanup = () => uvInflight.delete(key);
                const finish = (resp) => {
                    resolveResp(resp);
                    setTimeout(cleanup, UV_INFLIGHT_TTL);
                };
                const originalFetch = async () => {
                    ...
                    if (['image', 'style', 'script', 'font'].includes(request.destination)) {
                        responseCtx.headers['cache-control'] = 'no-store, no-cache, must-revalidate, proxy-revalidate, max-age=0';
                        responseCtx.headers['pragma'] = 'no-cache';
                        responseCtx.headers['expires'] = '0';
                    }
    ...
    getBarerResponse(response) {
        const headers = {};
        let rawHeader = response.headers.get('x-bare-headers');
        if (!rawHeader) {
            const parts = [];
            for (const [name, value] of response.headers.entries()) {
                if (!name.startsWith('x-bare-headers-')) continue;
                if (!value.startsWith(';')) continue;
                const index = parseInt(name.slice('x-bare-headers-'.length), 10);
                if (!Number.isNaN(index)) parts[index] = value.slice(1);
            }
            if (parts.length) rawHeader = parts.join('');
        }

        const statusHeader = response.headers.get('x-bare-status');
        const status = Number(statusHeader);

        if (!rawHeader || !Number.isFinite(status) || status < 100 || status > 599) {
            const fallbackStatus = response.status >= 200 && response.status <= 599 ? response.status : 502;
            console.warn('[uv] missing bare headers', {
        ...
        const raw = JSON.parse(rawHeader);
