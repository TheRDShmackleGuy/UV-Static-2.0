    ...
    client.override(window, 'open', (target, that, args) => {
        ...
        try {
            if (typeof rawUrl === 'string') {
                rawUrl = new URL(rawUrl, __uv.location.href).href;
            }
        } catch {}

        const rewritten = __uv.rewriteUrl(rawUrl);

        try {
            const hostWin = (window.top && window.top.EQNX_POPUP) ? window.top : (window.parent || null);
            if (hostWin && hostWin.EQNX_POPUP && typeof hostWin.EQNX_POPUP.open === 'function') {
                hostWin.EQNX_POPUP.open({ rawUrl, proxiedUrl: rewritten });
                return null;
            }
        } catch {}

        return target.call(that, rewritten);
    });
